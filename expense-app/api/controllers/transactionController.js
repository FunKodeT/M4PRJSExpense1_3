...
const transaction_post = async (req, res) => {
	...
	const schema = z.object({
		title: z
			.string()
			.min(2, {msg: 'Title must be atleast 2 characters in length'}),
		money: z
			.number()
			.positive({msg: 'Money must consist of a positive value'}),
	...
	if (isValid.error) {
		res.status(400).json({
			...
			message: 'Please provide input in all available fields',
		});
		return;
	}
